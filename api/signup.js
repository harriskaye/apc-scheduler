...
//
// v2 (Aug 2026): role matching is now whitespace-proof. The Role and
// Assigned To values are TRIMmed inside the Airtable formula, so stray
// leading/trailing spaces picked up during role renames can no longer
// make every open role answer "taken." Intern/External guards unchanged.
...
module.exports = async (req, res) => {
  ...
  try {
    // find ALL open rows for this role (handles numbered duplicate slots).
    // TRIM(...&"") makes the match immune to invisible spaces in Airtable
    // cells; the &"" coerces a truly blank cell to a string before trimming.
